// Keep shared V2 resources while routing visitors within the selected draft.
const body = document.body;
const v2 = new URL('../', import.meta.url);
...
function routeLinks() {
    document.querySelectorAll('a[href]').forEach(link => {
        ...
        let target = url.href;
        if (url.href.startsWith(about.href)) return;
        if (url.origin === v2.origin && url.pathname.startsWith(v2.pathname)) {
            const path = url.pathname.slice(v2.pathname.length);
