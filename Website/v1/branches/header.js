...
function renderHeader() {
    ...
    const aboutHref = new URL(`../${document.body.dataset.slug}/${document.body.dataset.variant === 'landing' ? 'landing/' : ''}about/`, import.meta.url).href;
    const linksHTML = [[new URL('../', import.meta.url).href, 'Main Branch'], [aboutHref, 'About']]
        .map(([href, label]) => {
            const isCta = href === 'join_us/';
            return `<a href="${href}" data-site-link class="${isCta ? 'nav-cta' : 'nav-link'}">${label}</a>`;
        })
        .join('');
