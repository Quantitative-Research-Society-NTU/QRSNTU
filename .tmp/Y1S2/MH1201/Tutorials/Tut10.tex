...
Part (a) follows because $H$ is Hermitian positive definite, so $\ip{u}{v}=u^\dagger Hv$ satisfies all inner product axioms.
For Gram--Schmidt, note that the projection coefficients are entries of the Gram matrix $H$ in the standard basis, making computations
particularly simple: $\ip{e_i}{e_j}=H_{ij}$. The resulting orthogonal vectors match Method 1, and normalization is immediate once
$\ip{u_k}{u_k}$ are computed.
