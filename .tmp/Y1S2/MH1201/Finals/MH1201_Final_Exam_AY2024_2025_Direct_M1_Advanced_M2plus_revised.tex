...
\begin{enumerate}[label=(\alph*)]
  ...
  \item The standard matrix representation obtained in Part (a) is
  \[
    A\df [D^2+D+I]_{\sigma}^{\sigma}
    =
    \begin{bmatrix}
      1&1&2\\
      0&1&2\\
    ...
    \end{bmatrix}.
  \]
  This matrix is upper triangular with nonzero diagonal entries $1,1,1$. Therefore
  \[
    \det(A)=1\cdot 1\cdot 1=1\ne 0.
  \]
  Equivalently, $A$ has full rank:
  \[
    \rank(A)=3=\dim\cP_2(\R).
  \]
  Hence the standard matrix representation is invertible. Therefore the linear map $D^2+D+I$ is invertible on $\cP_2(\R)$.
  \[
  ...
  \[
    (D^2+D+I)(P)
    =
    (\alpha+\beta+2\gamma)+(\beta+2\gamma)X+\gamma X^2.
  \]
  Equating this with $a+bX+cX^2$ gives
  \[
    \gamma=c,\qquad
    \beta+2\gamma=b,\qquad
  ...
  \[
    ...
    (a-b)+(b-2c)X+cX^2.}
  \]
  Equivalently, if $Q\in\cP_2(\R)$, then this is the same as
  \[
    (D^2+D+I)^{-1}(Q)=Q-Q'.
  \]
  ...
  \[
    (D^2+D+I)(P)=-2+3X-5X^2.
  \]
  In the formula from Part (c), take
  \[
...
\newpage
\subsubsection{Method 2: One-Round Polynomial-Coefficient Map Computation}
\begin{enumerate}[label=(\alph*)]
  \item Let
  \[
    f(X)=a+bX+cX^2.
  ...
  \[
    ...
    (a+b+2c)+(b+2c)X+cX^2.
  \]
  Therefore, relative to the standard ordered basis $\sigma=(1,X,X^2)$,
  \[
    \begin{bmatrix}
      a\\b\\c
    \end{bmatrix}
  ...
  \[
    ...
    \end{bmatrix}
  \]
  is upper triangular with nonzero diagonal entries. Hence its determinant is the product of its diagonal entries:
  \[
    1\cdot 1\cdot 1=1\ne 0.
  \]
  Therefore it has full rank and is invertible. Hence $D^2+D+I$ is invertible.
  \[
    \boxed{D^2+D+I\text{ is invertible}.}
  \]

  \item Let the input polynomial be
  \[
    q(X)=a+bX+cX^2.
  \]
  From Part (a), if
  \[
    (D^2+D+I)(\alpha+\beta X+\gamma X^2)=a+bX+cX^2,
  \]
  then
  \[
    \gamma=c,\qquad \beta=b-2c,\qquad \alpha=a-b.
  \]
  Therefore
  \[
    \boxed{
  ...
  \item Substituting $a=-2$, $b=3$, and $c=-5$ into the inverse formula gives
  \[
    P(X)=(-2-3)+(3-2(-5))X-5X^2.
  \]
  Therefore
  \[
    \boxed{P(X)=-5+13X-5X^2.}
...
\newpage
\subsubsection{Method 3: Nilpotent Geometric-Series Theorem}
\begin{enumerate}[label=(\alph*)]
...
\newpage
\subsubsection{Method 4: Upper-Triangular Recursion Theorem}
\begin{enumerate}[label=(\alph*)]
  \item For
  \[
    P(X)=a+bX+cX^2,
  ...
  \]
  The last output coordinate is exactly $c$, the second output coordinate determines $b$ once $c$ is known, and the first output coordinate determines $a$ once $b,c$ are known. Equivalently, the standard matrix is upper triangular with all diagonal entries nonzero, hence has full rank. Thus the map is invertible.
  \[
    \boxed{D^2+D+I\text{ is invertible}.}
  \]
  ...
  solve
  \[
    (\alpha+\beta+2\gamma)+(\beta+2\gamma)X+\gamma X^2=a+bX+cX^2.
  \]
  ...
  Therefore
  \[
    \boxed{(D^2+D+I)^{-1}(a+bX+cX^2)=(a-b)+(b-2c)X+cX^2.}
  \]
...
\begin{enumerate}[label=(\alph*)]
  ...
  \[
    ...
    a+(a+b+c)X+(a+b+c)X^2.
  \]
  Thus, relative to the standard ordered basis $\sigma=(1,X,X^2)$,
  \[
    \begin{bmatrix}
      a\\b\\c
    \end{bmatrix}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \]
  Thus the given inner product is the ordinary Euclidean dot product after the
  invertible change of variables
  \[
    x\mapsto Rx.
  \]
  Since $R$ is invertible, $Rx=0$ implies $x=0$. Hence
  \[
    \ip{x}{x}=(Rx)\cdot(Rx)=\|Rx\|_2^2\ge0,
  \]
  with equality if and only if $x=0$. Bilinearity and symmetry follow from
  bilinearity and symmetry of the Euclidean dot product. Therefore
  \[
    \boxed{\ip{\cdot}{\cdot}\text{ is an inner product}.}
  \]

  \item The whitening identity
  \[
    \ip{x}{y}=(Rx)\cdot(Ry)
  \]
  means that applying Gram--Schmidt to $\{e_1,e_2,e_3\}$ under $\ip{\cdot}{\cdot}$
  is equivalent to applying ordinary Euclidean Gram--Schmidt to
  \[
    Re_1,\quad Re_2,\quad Re_3,
  \]
  then pulling the result back by $R^{-1}$.
  ...
  Let
  \[
    w_1=Re_1,\qquad w_2=Re_2,\qquad w_3=Re_3.
  \]
  We now apply ordinary Euclidean Gram--Schmidt to $(w_1,w_2,w_3)$.

  For the first vector,
  \[
    \widetilde q_1=w_1=\begin{bmatrix}1\\0\\0\end{bmatrix},
    \qquad
    \|\widetilde q_1\|_2=1,
  \]
  so
  \[
    q_1=\frac{\widetilde q_1}{\|\widetilde q_1\|_2}
    =
    \begin{bmatrix}1\\0\\0\end{bmatrix}.
  \]
  ...
  \[
    \widetilde q_2=w_2-(w_2\cdot q_1)q_1.
  \]
  Since
  \[
    w_2\cdot q_1=
    \begin{bmatrix}0\\\sqrt2\\0\end{bmatrix}
    \cdot
    \begin{bmatrix}1\\0\\0\end{bmatrix}
    =0,
  \]
  we get
  \[
    \widetilde q_2=w_2=\begin{bmatrix}0\\\sqrt2\\0\end{bmatrix}.
  \]
  Hence
  \[
    \|\widetilde q_2\|_2=\sqrt2,
    \qquad
    q_2=\frac{\widetilde q_2}{\|\widetilde q_2\|_2}
    =
  ...
  \[
    \widetilde q_3
    =
    w_3-(w_3\cdot q_1)q_1-(w_3\cdot q_2)q_2.
  \]
  Now
  \[
    w_3\cdot q_1=
  ...
  we have
  \[
    q_3=\frac{\widetilde q_3}{\|\widetilde q_3\|_2}
    =
    \begin{bmatrix}0\\0\\1\end{bmatrix}.
  \]

  Thus ordinary Euclidean Gram--Schmidt gives
  \[
    (q_1,q_2,q_3)
    =
    \left(
  ...
  Now pull this Euclidean orthonormal basis back to the original space. Since
  \[
    R^{-1}=
    \begin{bmatrix}
      1&0&-1\\
  ...
  Finally, this basis is orthonormal with respect to $\ip{\cdot}{\cdot}$ because
  \[
    R\beta_i=q_i
    \qquad (i=1,2,3),
  \]
  ...
  Since
  \[
    R\beta_1=q_1,\qquad R\beta_2=q_2,\qquad R\beta_3=q_3,
  \]
  the $\beta$-Fourier coefficients of $v$ under $\ip{\cdot}{\cdot}$ equal the
  ordinary Euclidean Fourier coefficients of $Rv$ under the orthonormal basis
  $(q_1,q_2,q_3)$.
  ...
  \]
  Since $(q_1,q_2,q_3)$ is the standard Euclidean orthonormal basis, the
  coefficients are directly the entries of $Rv$:
  \[
    c_1=4,
    \qquad
    c_2=2\sqrt2,
...
\newpage
\subsubsection{Method 2: Direct Orthogonal-Complement Characterisation}
\begin{enumerate}[label=(\alph*)]
  \item Let $A,B\in\R^{n,n}$. Then
  \[
    \ipF{MA}{MB}
  ...
  \item We prove $V^\perp=W$ directly from the definition of orthogonal complement.

  First, let $K\in W$, so $K^\T=-K$. For any $S\in V$, so $S^\T=S$, we have
  \[
    \ipF{S}{K}
    =
    \tr(S^\T K)
  ...
  Conversely, let $C=(c_{ij})\in V^\perp$. We show that $C$ must be skew-symmetric. Since every diagonal matrix unit $E_{ii}$ is symmetric and lies in $V$,
  \[
    0=\ipF{E_{ii}}{C}=\tr(E_{ii}^\T C)=c_{ii}.
  \]
  Hence
  \[
    c_{ii}=0
    \qquad(1\le i\le n).
  \]
  Next, for $i<j$, the matrix
  \[
    E_{ij}+E_{ji}
  \]
  is symmetric and therefore belongs to $V$. Since $C\in V^\perp$,
  \[
    0
    =
    \ipF{E_{ij}+E_{ji}}{C}
  ...
  \[
    ...
    \qquad(i<j).
  \]
  Combining the diagonal condition $c_{ii}=0$ with the off-diagonal condition $c_{ji}=-c_{ij}$ gives
  \[
    C^\T=-C.
  \]
  Therefore $C\in W$, so
...
\newpage
\subsubsection{Method 3: Orthogonal Direct-Sum Decomposition Theorem}
\begin{enumerate}[label=(\alph*)]
...
\newpage
\subsubsection{Method 4: Matrix-Unit Orthogonal Basis Theorem}
\begin{enumerate}[label=(\alph*)]
...
\newpage
\subsubsection{Method 5: Self-Adjoint Involution Eigenspace Theorem}
\begin{enumerate}[label=(\alph*)]
